...
function getMethodOddsLines(pokemonObj, caughtSpecies, prefix = "") {
  const lines = [];
  if (Array.isArray(pokemonObj)) {
    const visibleMons = pokemonObj.filter((m) => m !== "No encounters");
    const eligibleMons = visibleMons.filter(
      (m) => !caughtSpecies.has(String(m).toLowerCase())
    );
    const odds =
      eligibleMons.length > 0
        ? eligibleMons.map((m) => `${m} (${(100 / eligibleMons.length).toFixed(1)}%)`).join(", ")
        : "All dupes / no eligible encounters";
    lines.push({ label: prefix || "encounters", text: odds, count: eligibleMons.length });
    return lines;
  }

  if (typeof pokemonObj === "object" && pokemonObj !== null) {
    Object.entries(pokemonObj).forEach(([key, value]) => {
      lines.push(...getMethodOddsLines(value, caughtSpecies, prefix ? `${prefix} ${key}` : key));
    });
  }
  ...
  function renderEncounterMethods(entry, caughtSpecies) {
    ...
    function traverse(obj, prefix = "") {
      if (Array.isArray(obj)) {
        methodElements.push(
          <p key={`${entry.area}-${prefix}`}>
            <strong>{prefix}:</strong> {obj.join(", ")}
          </p>
        );

        const visibleMons = obj.filter((m) => m !== "No encounters");
        const eligibleMons = visibleMons.filter(
          (m) => !caughtSpecies.has(String(m).toLowerCase())
        );

        methodElements.push(
          <p key={`${entry.area}-${prefix}-odds`} style={{ marginTop: -8, color: "#555", fontSize: 13 }}>
            <strong>{prefix} odds:</strong>{" "}
            {eligibleMons.length > 0
              ? eligibleMons.map((m) => `${m} (${(100 / eligibleMons.length).toFixed(1)}%)`).join(", ")
              : "All dupes / no eligible encounters"}
          </p>
            ...
            return (
              ...
              >
                ...
                <p style={{ marginTop: -6, color: "#666", fontSize: 13 }}>
                  Eligible encounters after Dupes Clause: {possibleMons.filter((m) => !caughtSpecies.has(m.toLowerCase())).length} / {possibleMons.filter((m) => m !== "No encounters").length}
                </p>
